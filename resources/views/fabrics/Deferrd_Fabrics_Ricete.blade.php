...
<form id="invoiceForm" action="{{ route('insert_deferred_fabrics_recite') }}" method="post">
  @csrf
  ...
  <div id="invoiceContainer">
    ...
    <!-- <label for="availableFabricRobe">عدد الأثواب المتاحة:</label>
      <input type="number" class="availableFabricRobe" name="availableFabricRobe[]" readonly><br>
      <label for="availableFabricMeter">عدد الأمتار المتاحة:</label>
      <input type="number" class="availableFabricMeter" name="availableFabricMeter[]" readonly><br>
    -->
      ...
      <input type="number" class="deferred" name="deferred[]" oninput="calculateTotal(this)"><br>

      <label for="period">الفترة:</label>
      <input type="number" class="period" name="period[]" oninput="calculateTotal(this)"><br>
...
*/
  function calculateTotal(input) {
    ...
    var deferred = parseFloat(parent.querySelector('.deferred').value) || 0;
    var meters = parseFloat(parent.querySelector('.much_of_fabricmeter').value) || 0;
    var period = parseFloat(parent.querySelector('.period').value) || 0;
    var total = price * quantity + (deferred * period * meters);
